...
I intend to use the neuralnet package in order to build my neural net mode. 
This code is modified from my homework 5 assignment so I will added additional annotations and 
...
quantile(expVar$variance, seq(0,1,by=0.1))
```
...
Based on my NN method which can handle noisy data I will only include the top 10% variable features to keep. 


```{r}
# Make boolean index
quant_idx <- expVar$variance > .98099 

# subset with index 
features.1 <- expression[quant_idx,]

dim(features.1)
```

Excellent we are now down to a relatively managable 1864 features. 

SUBTYPE FEATURE SELECTION GOES HERE


OR TWO BOOLEAN INDEXS TOGETHER TO GET THE ADDITION OF THE NORMAL SUBTYPSE
...
# Now append the matrixes in order as they appear node amount. 
...
mat_zip <- function( mat1, mat2, id_list=list()) {
  ...
  for (i in seq(n)){
    ...
    nodes <- as.character(id_list$nodes[i])
    #print(nodes)
...
# FIGURE OUT HOW TO PUT THE TITLE IN
